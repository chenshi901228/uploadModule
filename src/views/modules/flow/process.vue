  ...
  <el-card shadow="never" class="aui-card--fill">
    <div class="mod__process">
      ...
      <el-table
        ...
        <el-table-column prop="id" :label="$t('process.id')" header-align="center" align="center" width="200"></el-table-column>
  ...
  methods: {
    ...
    activeHandle (id) {
      this.$confirm(this.$t('prompt.info', { 'handle': this.$t('process.active') }), this.$t('prompt.title'), {
        ...
        this.$http.put(`/flow/process/active/${id}`).then(({ data: res }) => {
          if (res.code !== 0) {
    ...
    suspendHandle (id) {
      this.$confirm(this.$t('prompt.info', { 'handle': this.$t('process.suspend') }), this.$t('prompt.title'), {
        ...
        this.$http.put(`/flow/process/suspend/${id}`).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          this.$message({
    ...
    convertToModelHandle (id) {
      this.$confirm(this.$t('prompt.info', { 'handle': this.$t('process.convertToModel') }), this.$t('prompt.title'), {
        ...
        this.$http.post(`/flow/process/convertToModel/${id}`).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          this.$message({
