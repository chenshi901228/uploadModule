<template>
  <el-card shadow="never" class="aui-card--fill">
    <div class="mod-sys__params">
      ...
      >
        <el-form-item :label="$t('params.paramCode')" prop="paramCode">
          <el-input size="small" v-model="dataForm.paramCode" :placeholder="$t('params.paramCode')" clearable></el-input>
        </el-form-item>
        <!-- 搜索重置展开按钮 -->
      ...
      >
        ...
        <el-table-column :label="$t('handle')" fixed="right" header-align="center" align="center" width="150">
          <template slot-scope="scope">
            <el-button v-if="$hasPermission('sys:params:update')" type="text" icon="el-icon-edit" size="mini" @click="addOrUpdateHandle(scope.row.id)">{{ $t('update') }}</el-button>
            <el-button v-if="$hasPermission('sys:params:delete')" type="text" icon="el-icon-delete" size="mini" @click="deleteHandle(scope.row.id)">{{ $t('delete') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
