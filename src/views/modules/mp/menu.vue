          ...
          <div class="weixin-preview">
            ...
            <div class="weixin-bd">
              <ul class="weixin-menu" id="weixin-menu">
                <li v-for="(btn, i) in menu.button" :key="i" class="menu-item" :class="{ current: selectedMenuIndex === i && selectedMenuLevel() === 1 }" @click="selectedMenu(i, $event)">
                  <div class="menu-item-title">
                    <i class="icon_menu_dot"></i>
                    <span>{{ btn.name }}</span>
                  </div>
                  <ul class="weixin-sub-menu" v-show="selectedMenuIndex === i">
                    <li v-for="(sub, i2) in btn.sub_button" :key="i2" class="menu-sub-item" :class="{ current: selectedSubMenuIndex === i2 && selectedMenuLevel() === 2 }" @click.stop="selectedSubMenu(i2, $event)">
                      <div class="menu-item-title">
                        <span>{{ sub.name }}</span>
                      </div>
                    </li>
...
export default {
  ...
  methods: {
    ...
    getMenu (appId) {
      this.$http.get('/sys/mp/menu/' + appId).then(({ data: res }) => {
        if (res.code !== 0) {
    ...
    init () {
      this.$http.get('/sys/mp/account/page').then(({ data: res }) => {
        if (res.code !== 0) {
          return this.$message.error(res.msg)
        }
    ...
    selectedMenu (i) {
      ...
      const selectedMenu = this.menu.button[this.selectedMenuIndex]
      // 清空选中media_id 防止再次请求
    ...
    selectedSubMenu (i) {
      ...
      const selectedSubMenu = this.menu.button[this.selectedMenuIndex].sub_button[this.selectedSubMenuIndex]
    ...
    getMaterial (mediaId) {
      ...
      this.$http.get('/sys/mp/material/get', {
        appId: this.appId,
        mediaId: mediaId
      }).then(({ data: res }) => {
        if (res.code !== 0) {
    ...
    getNewsList () {
      ...
      this.$http.get('/sys/mp/material/page', {
        appId: this.appId,
        type: 'news',
    ...
    async onMenuSubmit () {
      ...
      this.$http.post('/sys/mp/menu/' + this.appId, dataForm).then(({ data: res }) => {
        if (res.code !== 0) {
          return this.$message.error(res.msg)
        }
        this.$message({
    ...
    async onMenuClear () {
      try {
        ...
        this.$http.delete('/sys/mp/menu/' + this.appId).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          this.$message({
