...
<template>
  <el-card shadow="never" class="aui-card--fill">
    <div class="mod-live__liveList">
      ...
      <!-- 主播弹框 -->
      <choose-anchor ref="chooseAnchor" @add="addAnchorConfirm" :anchorId="anchorId"></choose-anchor>
      <!-- 商品弹框 -->
      <choose-product
        ref="chooseProduct"
...
export default {
  ...
  methods: {
    ...
    getDynamicAssistantList() {
      this.$http
        .get(
          `/sys/anchorAssistant/live/getAnchorAssistantWithLiveByAnchorId?anchorId=${this.anchorId}`
        )
        .then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg);
    ...
    getDynamicGroupList() {
      this.$http
        .get(`/sys/dynamicGroup/getAllDynamicGroupList?anchorId=${this.anchorId}`)
        .then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg);
          }
