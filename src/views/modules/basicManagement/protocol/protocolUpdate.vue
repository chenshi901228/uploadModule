...
export default {
    ...
    methods: {
        // 初始化获取协议详情
        init(id) {
            if(!id) return this.$message.error("获取协议详情失败，请重试")
            this.$http.get("/sys/sysConsultativeManagement/" + id).then(({data: res}) => {
                if(res.code != 0) return this.$message.error(res.msg)
                this.dataForm = res.data
                this.quillEditor.pasteHTML(res.data.introduce)
        ...
        onSubmit() {
            ...
            this.$refs.dataForm.validate((valid) => {
                if(valid) {
                    ...
                    this.$http.put("/sys/sysConsultativeManagement", {...this.dataForm, ...obj}).then(({data: res}) => {
