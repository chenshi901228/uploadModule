...
<template>
    <div>
        <el-card shadow="never" class="aui-card--fill">
                ...
                <el-row>
                    ...
                    <el-col :span="24">
                        <el-form-item style="float:right; padding-right:10px">
                            <el-button :disabled="!dataListSelections.length" size="small" type="primary" @click="upGoods()">批量上架</el-button>
            ...
            >
                ...
                >
                    <template slot-scope="{ row }">
                        ...
                        <div v-if="item.prop == 'productImage'">
                            <img style="width: 100%;height: 60px" class="frontCoverImg" :src="row.productImage || 'https://picsum.photos/400/300?random=1'" alt="">
                        </div>
                        <!-- 商品价格 -->
                ...
                <el-table-column :label="$t('handle')" fixed="right" header-align="center" align="center" width="100">
                    <template slot-scope="{ row }">
                        <el-button type="text" size="small" @click="upGoods(row.id)">上架</el-button>
                        <el-button style="margin-left: 10px" type="text" size="small" @click="emdit(row)">编辑</el-button>
                    </template>
                </el-table-column>
            </el-table>
            <el-pagination
        ...
        <!-- 编辑 -->
        <goods-add-or-update ref="goodsAddOrUpdate" @refreshDataList="getDataList"></goods-add-or-update>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        getGoodsName(s) {
            if(s != ""){
                this.loading = true
                this.$http.get("/sys/course/searchByNameWithDel", {
                    params: { 
                        productName: s
                    }
                }).then(({ data: res }) => {
        ...
        upGoodsHandle(data) {
            this.$http.post("/sys/course/up", data).then(({ data: res }) => {
                if(res.code == 0) {
        ...
        upGoods(id){
            if(!id) { //批量上架
                if(!this.dataListSelections.length){ //未勾选任何选项
