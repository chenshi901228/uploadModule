        ...
        <div class="mod-live__hotSearchConfig">
            <el-form class="headerTool" :inline="true" :model="dataForm" ref="dataForm" size="small" label-width="100px"
                ...
                <el-form-item label="配置内容" prop="value">
                    <el-input style="width: 200px" v-model.trim="dataForm.value" placeholder="配置内容" clearable>
                    </el-input>
                </el-form-item>
                <el-form-item label="分类" prop="type">
                    <el-select style="width: 200px" v-model="dataForm.type" placeholder="分类" clearable>
                ...
                <div class="headerTool-handle-btns">
                    <div class="headerTool--handle-btns-left">
                        <el-form-item>
                            <el-button size="mini" icon="el-icon-plus" type="primary" plain @click="add()">新增
                            </el-button>
                        </el-form-item>
                    </div>
            ...
            <el-table v-loading="dataListLoading" :data="dataList" @selection-change="dataListSelectionChangeHandle"
                :height="siteContentViewHeight" style="width: 100%" @cell-dblclick="cellDblclick">
                <el-table-column header-align="center" align="center" v-for="item in tableItem" :key="item.prop"
                    ...
                    <template slot-scope="{ row }">
                        <el-input-number ref="editRef" size="small" style="width:100px" @blur="editHandle" :min="0"
                        :controls="false"
                            :precision="0" :max="9999" v-if="editId == row.id && item.prop == 'clickNum'"
                            v-model="editData.clickNum" clearable>
                        </el-input-number>
                        <!-- <span v-else-if="item.prop == 'clickNum'">
                            {{row[item.prop]}}<i style="margin-left: 10px" class="el-icon-edit"></i>
                        </span> -->
                        <span v-else-if="item.prop == 'type'">
                            {{ row[item.prop] == 0 ? "预告" : row[item.prop] == 1 ? "直播" : row[item.prop] == 2 ? "视频" :
                                    "主播"
                            }}
                        </span>
                ...
                <el-table-column :label="$t('handle')" fixed="right" header-align="center" align="center">
                    <template slot-scope="{ row }">
                        <el-button type="text" size="small" icon="el-icon-delete" @click="deleteCount(row)">移除
                        </el-button>
                        <el-button type="text" size="small" icon="el-icon-sort-down" v-if="row.num != dataList.length"
                            @click="upOrDownHandle(row, -1)">下移</el-button>
                        <el-button type="text" size="small" icon="el-icon-sort-up" v-if="row.num != '1'"
