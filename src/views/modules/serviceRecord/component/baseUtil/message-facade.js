...
export function parseText(message) {
 ...
 //解析自定义消息
 export function parseCustomMsg(message){
	 const { payload } =message;
	 let data = JSON.parse(payload.data)
	 return data
 }
 // 解析小程序text, 表情信息也是[嘻嘻]文本
 export function parseTextWithMySend(message) {
   const renderDom = [];
   let temp = message;
   let left = -1;
   let right = -1;
...
export function parseGroupTip(message) {
  ...
  let userNameArr = [];
  payload.userIDList && payload.userIDList.forEach(userInfo => {
	  let tempArr = userInfo.split('_LIVE_FLAG_');
	  userNameArr.push(tempArr.length > 1 ? tempArr[1] : tempArr[0])
  })
  const userName = message.nick || userNameArr.join(',');
  let tip;
  let user;
